...
// Usamos 'var' en lugar de 'const' o 'let' para evitar errores de duplicación si quedaron variables en los HTMLs.
...
const globalUI = `
    ...
    <div id="searchOverlay" class="modal-overlay" onclick="if(event.target === this) closeSearch()">
        <div class="search-content">
            ...
            <div id="globalSearchResults" class="search-results hidden">
                <p class="text-xs text-gray-400 text-center py-8 font-medium">Escribe algo para comenzar...</p>
            </div>
        </div>
    </div>
    ...
    <header id="mainHeader" class="h-16 px-6 lg:px-10 flex items-center justify-between sticky top-0 bg-white/80 backdrop-blur-md z-50 border-b border-gray-100 no-export">
        ...
        <div class="flex items-center gap-3 ml-auto relative">
            ...
            <button onclick="openSearch()" class="hidden md:flex items-center gap-2 px-3 py-1.5 bg-gray-100 hover:bg-gray-200 border border-gray-200 rounded-full transition text-gray-500 mr-2 group">
                ...
                <span class="text-[10px] font-bold tracking-widest">Buscar... <span class="text-gray-400 font-medium ml-1">Ctrl K</span></span>
            </button>
...
// --- FUNCIÓN INTELIGENTE DE INYECCIÓN DE UI ---
function injectGlobalUI() {
    // Verificamos que no se haya inyectado ya para evitar duplicados
    if (!document.getElementById('mainHeader')) {
        ...
        // Asegurar que los íconos inyectados se dibujen
        if(typeof lucide !== 'undefined') lucide.createIcons();
    }
}

// Escuchar a que el navegador esté listo antes de inyectar, previniendo pantallas en blanco
...
async function initCore() {
    // Inicializar Supabase en el momento exacto en que se necesita
    if (!_supabase) {
    ...
    if(profile) {
        ...
        // Esperamos a que los IDs existan antes de escribirles
...
async function loadNotifications() {
    if(!document.getElementById('notif-list')) return; // Evita errores si UI no cargó
...
document.addEventListener('keydown', (e) => {
    ...
    if (e.key === 'Escape' && document.getElementById('searchOverlay') && document.getElementById('searchOverlay').classList.contains('active')) {
        closeSearch();
    }
});
...
function closeSearch() {
    ...
    if(overlay) {
        ...
        document.getElementById('globalSearchResults').innerHTML = '<p class="text-xs text-gray-400 text-center py-8 font-medium">Escribe algo para comenzar...</p>';
    }
}
...
function executeSearch(query) {
    ...
    if (!query || query.trim().length < 2) {
        resultsContainer.innerHTML = '<p class="text-xs text-gray-400 text-center py-8 font-medium">Escribe al menos 2 letras para buscar...</p>';
        return;
    }
    ...
    searchTimeout = setTimeout(async () => {
        ...
        // Búsqueda cruzada en las tablas usando el SDK
        ...
        if (reqs.data && reqs.data.length > 0) {
            ...
            reqs.data.forEach(r => {
                html += `<a href="requerimientos.html" class="flex items-center gap-3 p-3 mx-2 my-1 bg-white hover:bg-gray-50 hover:border-gray-200 border border-transparent rounded-xl transition cursor-pointer group shadow-sm">
                    <div class="w-8 h-8 bg-blue-50 text-blue-500 rounded-xl flex items-center justify-center shrink-0 group-hover:scale-110 transition"><i data-lucide="layers" class="w-4 h-4"></i></div>
                    <div class="flex-1 overflow-hidden"><p class="text-sm font-bold text-gray-900 truncate leading-snug">${r.title}</p><p class="text-[9px] text-gray-400 uppercase tracking-widest mt-0.5 font-bold">Nivel: <span class="prio-${r.priority} px-1 rounded">${r.priority}</span></p></div>
                    <i data-lucide="arrow-right" class="w-4 h-4 text-gray-300 group-hover:text-black transition transform group-hover:translate-x-1"></i>
                </a>`;
            });
        }
        ...
        if (hitos.data && hitos.data.length > 0) {
            ...
            hitos.data.forEach(h => {
                html += `<a href="plan_trabajo.html" class="flex items-center gap-3 p-3 mx-2 my-1 bg-white hover:bg-gray-50 hover:border-gray-200 border border-transparent rounded-xl transition cursor-pointer group shadow-sm">
                    <div class="w-8 h-8 bg-purple-50 text-purple-500 rounded-xl flex items-center justify-center shrink-0 group-hover:scale-110 transition"><i data-lucide="flag" class="w-4 h-4"></i></div>
                    <div class="flex-1 overflow-hidden"><p class="text-sm font-bold text-gray-900 truncate leading-snug">${h.title}</p><p class="text-[9px] text-gray-400 font-bold mt-0.5">Módulo: Plan de Trabajo</p></div>
                    <i data-lucide="arrow-right" class="w-4 h-4 text-gray-300 group-hover:text-black transition transform group-hover:translate-x-1"></i>
                </a>`;
            });
        }
        ...
        if (tasks.data && tasks.data.length > 0) {
            ...
            tasks.data.forEach(t => {
                html += `<a href="plan_trabajo.html" class="flex items-center gap-3 p-3 mx-2 my-1 bg-white hover:bg-gray-50 hover:border-gray-200 border border-transparent rounded-xl transition cursor-pointer group shadow-sm">
                    <div class="w-8 h-8 bg-green-50 text-green-600 rounded-xl flex items-center justify-center shrink-0 group-hover:scale-110 transition"><i data-lucide="check-square" class="w-4 h-4"></i></div>
                    <div class="flex-1 overflow-hidden"><p class="text-sm font-bold text-gray-900 truncate leading-snug">${t.title}</p><p class="text-[9px] text-gray-500 uppercase tracking-widest mt-0.5 font-bold">Resp: ${t.assign} • Est: <span class="status-${t.status} px-1 rounded">${t.status}</span></p></div>
                    <i data-lucide="arrow-right" class="w-4 h-4 text-gray-300 group-hover:text-black transition transform group-hover:translate-x-1"></i>
                </a>`;
            });
        }
        ...
        if (html === '') {
            html = '<div class="py-12 text-center"><i data-lucide="search-x" class="w-10 h-10 text-gray-300 mx-auto mb-3"></i><p class="text-sm font-bold text-gray-600">No hay coincidencias</p><p class="text-xs text-gray-400 mt-1">Intenta buscar con otras palabras clave.</p></div>';
        }
